...
def parse_args():
    parser = argparse.ArgumentParser(description='Configuration editor for Proxy Checker')
...
def edit_config(config_file):
    ...
    config["proxy_file"] = input(f"Proxy file [{config['proxy_file']}]: ") or config["proxy_file"]
    config["output_file"] = input(f"Output file [{config['output_file']}]: ") or config["output_file"]
    config["test_url"] = input(f"Test URL [{config['test_url']}]: ") or config["test_url"]
    ...
    try:
        concurrent_input = input(f"Number of concurrent checks [{config['concurrent_checks']}]: ")
        if concurrent_input:
    ...
    save_input = input(f"Save working proxies to input file (y/n) [{('y' if save_to_input_file else 'n')}]: ")
    if save_input.lower() in ['y', 'yes', 'так', 'т']:
        config["save_to_input_file"] = True
    elif save_input.lower() in ['n', 'no', 'ні', 'н']:
